...
namespace BackendApp.Service
{
    ...
    (ApiContext dbContext, ILinkedOutPostService postService, ILinkedOutJobService jobService, ILinkedOutUserService userService) 
    : IInterestService
    {
        ...
        {
            LinkedOutUser? userInDb = this.userService.GetUserById(userId);
        ...
        public UpdateResult DeclareInterestForJob(uint userId, uint jobId)
        {
            ...
            return this.DeclareInterestFor(this.dbContext.LinkedOutJobs, userId, jobInDb);
        }
        public UpdateResult RemoveInterestForJob(uint userId, uint jobId)
        {
            var jobInDb = this.jobService.GetJobById(jobId);
            if(jobInDb is null) return UpdateResult.NotFound;
            return RemoveInterestFor(this.dbContext.LinkedOutJobs, userId, jobInDb);
        }
        public UpdateResult DeclareInterestForPost(uint userId, uint jobId)
        {
            ...
            return this.DeclareInterestFor(this.dbContext.LinkedOutPosts, userId, postInDb);
        }
        public UpdateResult RemoveInterestForPost(uint userId, uint jobId)
        {
            var postInDb = this.postService.GetPostById(jobId);
            if(postInDb is null) return UpdateResult.NotFound;
            return this.RemoveInterestFor(this.dbContext.LinkedOutPosts, userId, postInDb);
        }
    }
}